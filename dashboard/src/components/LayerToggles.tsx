...
const LAYER_CONFIG = [
  {
    key: "activeSatellites" as const,
    label: "Active Satellites",
    color: "bg-cyan-400",
  },
  { key: "debris" as const, label: "Debris", color: "bg-gray-400" },
  {
    key: "rocketBodies" as const,
    label: "Rocket Bodies",
    color: "bg-yellow-400",
  },
  ...
  {
    key: "conjunctionHighlights" as const,
    label: "Conjunctions",
    color: "bg-red-500",
  },
];
      ...
      <div className="space-y-1.5">
        {LAYER_CONFIG.map(({ key, label, color }) => {
          ...
          return (
            ...
            >
              <div
                className={`w-2.5 h-2.5 rounded-full ${color} transition-opacity ${isVisible ? "opacity-100" : "opacity-20"}`}
              />
      ...
      <div className="mt-4 pt-3 border-t border-white/10 space-y-2">
        <button
          onClick={() =>
            setLayerVisibility(
              "mapStyle",
              layerVisibility.mapStyle === "day" ? "night" : "day",
            )
          }
        ...
        <button
          onClick={() =>
            setLayerVisibility("autoRotate", !layerVisibility.autoRotate)
          }
          className="w-full flex items-center justify-between px-2 py-1.5 rounded text-xs text-slate-300 hover:bg-white/5 transition-all"
        >
